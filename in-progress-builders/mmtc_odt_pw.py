...
It expects odt_data to exist and to provide conRates, odt_data.conODTs, conPaths, path_to_dmd, etc.
"""
...
def build(data, odt_data, minProb = 0.8):
    ...
    model = build_revenue_and_costs(model, data, odt_data, vars, minProb=minProb, piecewise=True)
    ...
    add_h_bounds_piecewise(model, data, vars)
    add_mode_restrictions(model, data, vars)

    model.update()
    return model, vars

def build_og(data, odt_data, conserv = 0.8, bigMTime = 7, maxLTLShip = 5):
    model  = gp.Model(name = 'PM_PW')
    vars = {}
    ...
    ##y_{l,m} are binary variables indicating if lane l uses mode m - THIS IS WHERE ONE CHANGE IS
    vars['y'] = model.addVars(data.arcMode, lb = 0, name = 'y', vtype = GRB.BINARY)
    ...
    ##f_(l,m) are integer variables capturing the additional frequency needed for leg l using mode m
    # if 'int' in modelType:
    vars['f'] = model.addVars(data.arcMode, lb = 0, name = 'f', vtype = GRB.INTEGER)
    ##h_{l,m} are continuous variables that capture the inverse of f_lm variables
    vars['h'] = model.addVars(data.arcs, name = 'h') 
    ...
    model.addConstrs(u[r] >= data.pathParams[r]['wgt']*gp.quicksum(odt_data.conRates[odt_data.pathDmd[r],t]*w[odt_data.pathDmd[r],t] 
                                                        for t in odt_data.conODTs[odt_data.pathDmd[r]])
                            - (1-x[r])*odt_data.conRMax[odt_data.pathDmd[r]]*data.pathParams[r]['wgt'] for r in odt_data.conPaths)
    ...
    #ensuring correct y-var is activated
    model.addConstrs(f[l,m] <= data.bigMVals[l,m]*y[l,m] for l,m in data.arcMode)
    ...
    model.addConstrs(1/7*gp.quicksum(1/odt_data.cVals[r,t]*(t-data.pathParams[r]['fixedTT'])*w[odt_data.pathDmd[r],t] for t in odt_data.conODTs[odt_data.pathDmd[r]] if (r,t) in odt_data.cVals)
                ...
                >= gp.quicksum(h[data.arcsConstrPaths[r][l]] for l in range(data.pathParams[r]['arcCt'])) 
    ...
    model.addConstrs(h[l] >= wCoefs[m][n][0]*f[l,m]+wCoefs[m][n][1]+1.5*(y[l,m]) 
                                                for l,m in data.arcMode for n in range(len(wCoefs[m])))

    model.addConstrs(h[l]>= 1/bigMTime for l in data.arcs)

        
    ##restricting modes on legs for certain routes
    ### truckload
    model.addConstrs(x[r] <= y[data.arcTL[r],0] for r in data.arcTL)
    ### LTL
    model.addConstrs(x[r] <= gp.quicksum(y[data.arcLTL[r],m] for m in data.modeDict[data.arcLTL[r]] if m!=0) for r in data.arcLTL)
